{{ config(
    ...
    cluster_by=['data_level', 'country_iso3', 'source_key', 'item_key']
) }}
...
base as (
    select
        to_hex(md5(
            ...
            coalesce(cast(e.year as string), '') || '|' ||
        ...
        {{ acf_place_scope_coalesce('g_nat', 'g_iso', 'iso.country_iso3', 'e.country_name') }} as place_scope,
        concat('emissions_', lower(replace(coalesce(e.element, cast(e.item_code as string), ''), ' ', '_'))) as metric,
    ...
    from {{ ref('int_faostat_emissions_conformed') }} e
