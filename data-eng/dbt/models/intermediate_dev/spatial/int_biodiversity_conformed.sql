...
with_admin0 as (
    ...
    qualify e.latitude is null
        ...
        or row_number() over (
            partition by coalesce(e.gbif_id, ''), coalesce(e.source_natural_key, '')
...
select
    ...
    coalesce(
        ...
        n.nearest_iso3,
        case when length(trim(n.country_code)) = 3 then upper(trim(n.country_code)) end,
        g_iso3.country_iso3,
        g_iso2.country_iso3
    ) as country_iso3,
...
left join iso3_geo g_iso3
    on length(trim(n.country_code)) = 3
   and g_iso3.country_iso3 = upper(trim(n.country_code))
left join iso2_geo g_iso2
    on length(trim(n.country_code)) = 2
   and g_iso2.country_iso2 = upper(trim(n.country_code))
