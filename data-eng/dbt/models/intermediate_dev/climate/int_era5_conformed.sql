...
src as (
    select *
    from {{ ref('stg_copernicus_era5') }}
    where longitude between -25 and 60
...
with_admin0 as (
    ...
    left join admin0_africa a
        on {{ geo_admin0_bbox_prefilter('e', 'a') }}
    qualify a.country_iso2 is null
        or row_number() over (
            partition by
                cast(e.latitude as string),
                cast(e.longitude as string),
                coalesce(cast(e.valid_time as string), ''),
                coalesce(cast(e.ensemble_member as string), ''),
...
left join iso2_geo g_country
    on g_country.country_iso2 = {{ geo_iso2_for_country_dim_join(
        'n.exact_iso2',
        'n.nearest_dist_m',
        'n.nearest_iso2',
        'n.grid_iso2',
