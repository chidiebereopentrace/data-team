...
- [~] Re-enable and tune the reranker (`TASKS.md` §2). Record top-k / final-k choices — reranker **on** (`cross_encoder`, top_k=20 configurable via `RAG_RERANKER_TOP_K`); A/B before/after harness built (`ml/rag/eval/run_rerank_ab.py`) and cross-encoder backend confirmed available. **Recording the numbers is blocked locally** by a query-embedding dimension mismatch (collection indexed at 384-dim, local query model emits 768-dim) → deferred to Week 4 QA on the provisioned environment. Status: `ml/rag/eval/rerank_ab_results.md`.
- [x] Validate namespace separation with test queries: confirm **zero cross-contamination** across geographies — ✅ **done Jul 15** via `python -m ml.rag.validate_namespaces`: `academic_papers` (32,315 / `academic_article`), `policies` (1,734 / `policy_document`), `news_public_reports` (3,568 / `public_report`) — each clean; split-math matches the 37,617-pt legacy backup exactly. Results in `ml/rag/eval/namespace_validation_results.md`. (Flagged: `news_data` has some `unknown`/continental geos → Data-team tagging follow-up.)
- [x] Full end-to-end integration test of the query → response pipeline — ✅ **done Jul 15**. New `ml/rag/chatbot/test_pipeline_integration.py` drives the real compiled LangGraph via `run_rag()` with external boundaries (retrievers, LLM, reranker, web fallback) mocked. Covers: data-query path (answer + citations + ACF signal), direct-answer-first backstop end-to-end, no-data structured gap, meta short-circuit (ACF HIGH), and `session_id` threading. 5 tests passing.
- [x] Response quality: **direct questions must return a direct answer before context/sources** — ✅ **done Jul 15**. Prompt already leads with the direct-answer rule; added a deterministic backstop `_strip_preamble_openers()` in `generator.py` (wired into `_clean_answer`) that strips academic/meta preambles ("Based on the context…", "It is important to note…", "The context shows…", "Unfortunately…") if the model still emits them. 13 new unit tests; full generator suite green (46 passed).
- [x] Fix citation rendering + enrich highest-impact source metadata (`TASKS.md` §4) — rendering ✅ done; **identification of missing-metadata records automated Jul 15** via new `ml/rag/find_missing_bibliography.py` (scanned all 37,617 research points: **22,331 / 59.4% lack a clickable link**; titles are 100% present so citations never break). Report: `ml/rag/eval/bibliography_gap_report.md`. Manual DOI/URI enrichment of the highest-impact subset remains a data-curation task (continues post-launch — the real gap is far larger than the original 200–600 estimate).
