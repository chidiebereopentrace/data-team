...
async def v1_chat_agribusinesses(body: ChatRequest):
    """POST /v1/chat/agribusinesses — Agribusinesses tier (cross-country, market volatility, exports)."""
    return await _plan_chat("Agribusinesses", body, uuid.uuid4().hex, export_enabled=True)


@router.post("/chat/integrated")
async def v1_chat_integrated(body: ChatRequest):
    """POST /v1/chat/integrated — Integrated tier (full access, category lens per message, exports)."""
    return await _plan_chat("Integrated", body, uuid.uuid4().hex, export_enabled=True)
