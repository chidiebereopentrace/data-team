...
from ml.rag.chatbot.sql_compiler import SqlRequest, compile_sql, sql_compiler_enabled
from ml.rag.chatbot.value_index import complete_enum, resolve_geography_iso3


def _legacy_generic_sql(
    *,
    table: str,
    class_code: str,
    iso_vals: list[str],
...
class GenericEngine(ClassEngine):
    ...
    def run_plan(
        ...
        *,
        ...
        iso_vals = list(hits.get("country_iso3") or ["GHA"])
        shape = "panel" if len(iso_vals) >= 2 else "series"
